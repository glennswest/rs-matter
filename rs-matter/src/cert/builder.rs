...
mod tests {
    ...
    /// Diagnostic: dumps both the TLV-encoded RCAC and its derived
    /// ASN.1 DER form to /tmp/rs-matter-rcac.{tlv,der}. Lets us run
    /// `openssl x509 -inform DER -in ...` and `chip-cert print-cert`
    /// against the output to pinpoint format divergences from CHIP.
    /// Only meaningful when `RSMATTER_DUMP_CERT=1` is set.
    #[test]
    fn test_dump_rcac_for_inspection() {
        if std::env::var("RSMATTER_DUMP_CERT").is_err() {
            return;
        }
        let crypto = test_only_crypto();
        let rcac_secret_key = unwrap!(crypto.generate_secret_key());
        ...
        std::fs::write("/tmp/rs-matter-rcac.tlv", &cert_buf[..len]).unwrap();
        let cert = CertRef::new(crate::tlv::TLVElement::new(&cert_buf[..len]));
        let mut der = vec![0u8; MAX_CERT_TLV_AND_ASN1_LEN];
        let der_len = cert.as_asn1(&mut der).unwrap();

        // Wrap our TBS+signature into a complete X.509 cert:
        //   SEQUENCE {
        //     TBS,
        //     SEQUENCE { AlgorithmId },
        //     BIT STRING (DER-encoded signatureValue: SEQUENCE { INTEGER r, INTEGER s })
        //   }
        // For the diagnostic, dump just the TBS DER — that's enough for
        // openssl asn1parse to walk it and reveal layout issues.
        std::fs::write("/tmp/rs-matter-rcac-tbs.der", &der[..der_len]).unwrap();
        std::fs::write("/tmp/rs-matter-rcac-sig.bin", cert.signature().unwrap()).unwrap();
        eprintln!(
            "wrote /tmp/rs-matter-rcac.tlv ({len} B) and /tmp/rs-matter-rcac-tbs.der ({der_len} B)"
        );
    }
